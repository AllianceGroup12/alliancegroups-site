...
(function () {
    ...
    form.addEventListener('submit', async function (e) {
      ...
      if (!name || !email || !service || !message) {
        [
          ...
          document.getElementById('contact-service'),
      ...
      window.location.href = `mailto:info@alliancegroups.com.au?subject=${subject}&body=${body}`;
      submitBtn.disabled = false;
      submitBtn.querySelector('span').textContent = 'Submit Proposal Request';
    });
  }
  ...
  /* ══════════════════════════════════════════════════════════
     ASBESTOS ORDER FORM — live Formspree submission
     ══════════════════════════════════════════════════════════ */
  const FORMSPREE_ASBESTOS = 'xlderjvq'; // same form, or create a separate one for asbestos

  const orderForm   = document.getElementById('asbestosOrderForm');
  ...
  if (orderForm) {
    orderForm.addEventListener('submit', async function (e) {
      ...
      const suburb = document.getElementById('order-suburb').value.trim();

      if (!name || !phone || !email || !suburb) {
        ['order-name','order-phone','order-email','order-suburb'].forEach(id => {
      ...
      const product  = document.getElementById('order-product')?.value || 'Not specified';
      ...
      formData.append('_subject', `🔔 NEW AUasbestos order — ${product} (${name}, ${suburb})`);
      formData.append('_replyto', email);

      try {
        const res = await fetch(`https://formspree.io/f/${FORMSPREE_ASBESTOS}`, {
          method: 'POST',
          headers: { 'Accept': 'application/json' },
          body: formData,
        });
        ...
        if (res.ok) {
          orderForm.reset();
          submitBtn.disabled = false;
          submitBtn.querySelector('span').textContent = 'Submit Order / Request Quote';
          if (orderSuccess) {
            orderSuccess.classList.add('show');
            orderSuccess.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            setTimeout(() => orderSuccess.classList.remove('show'), 8000);
          }
          if (typeof gtag === 'function') {
            gtag('event', 'conversion', { 'send_to': 'AW-18006768389/asbestos_order' });
          }
          return;
        }
      } catch (_) { /* fall through */ }
      ...
      // Fallback: mailto
      const subject = encodeURIComponent(`AUasbestos order — ${product} (${name})`);
      const body    = encodeURIComponent(`Name: ${name}\nPhone: ${phone}\nEmail: ${email}\nSuburb: ${suburb}\nProduct: ${product}\nAddress: ${address}\nNotes: ${notes}`);
      const mailtoLink = `mailto:info@alliancegroups.com.au?subject=${subject}&body=${body}`;
  ...
  /* ── Product Buy Button Auto-Select & URL Parameter Autofill ── */
  function autoSelectProduct(prodName) {
    const prodSelect = document.getElementById('order-product');
    if (prodSelect && prodName) {
      for (let i = 0; i < prodSelect.options.length; i++) {
        if (prodSelect.options[i].value.toLowerCase().includes(prodName.toLowerCase()) || 
            prodSelect.options[i].text.toLowerCase().includes(prodName.toLowerCase())) {
  ...
  document.querySelectorAll('.product-buy-btn').forEach(btn => {
    btn.addEventListener('click', function () {
      ...
      autoSelectProduct(prodName);
      document.getElementById('asbestosOrderForm')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });

  /* ── URL Query Parameters Handler (Auto-fill & Auto-select) ── */
  (function handleUrlParams() {
    if (!window.location.search) return;
    const params = new URLSearchParams(window.location.search);
    
    // Service category pre-selection for contact.html
    const serviceParam = params.get('service');
    const contactService = document.getElementById('contact-service');
    if (serviceParam && contactService) {
      for (let i = 0; i < contactService.options.length; i++) {
        if (contactService.options[i].value === serviceParam || 
            contactService.options[i].value.toLowerCase().includes(serviceParam.toLowerCase())) {
    ...
    // Product pre-selection for asbestos-services.html
    const productParam = params.get('product');
    if (productParam) {
      autoSelectProduct(productParam);
    }

    // Project & Notes pre-fill
    const projectParam = params.get('project');
    const notesParam = params.get('notes');
    const messageInput = document.getElementById('contact-message');
    if (messageInput) {
      if (projectParam && !messageInput.value) {
        const projectNames = {
          'river-terrace': 'River Terrace Residences (Residential Luxury)',
          'riverside-tower': 'Riverside Commercial Tower (Commercial Fitout)',
          'forest-retreat': 'The Forest Retreat (Architectural Estate)',
          'indooroopilly': 'Indooroopilly Transformation (Heritage Renovation)'
        };
        const projTitle = projectNames[projectParam] || projectParam;
        messageInput.value = `Hi Alliance Group,\n\nI am inquiring about a project similar to "${projTitle}". Please get in touch regarding scope, compliance, and estimation.`;
      } else if (notesParam && !messageInput.value) {
        messageInput.value = decodeURIComponent(notesParam);
      }
    }
  })();
